...
module Searchable
  ...
  included do
    ...
    def self.search(query, filter_regions, filter_countries, filter_states, filter_cities, filter_lang, with_explain)
        ...
        I18n.available_locales.each do |locale|
          %i[main_topic bio website].each do |name|
            indexes :"#{name}_#{locale}", type: 'text', analyzer: "#{ANALYZERS[locale]}_without_stemming" do
              indexes :suggest, type: 'completion', contexts: [{name: 'region', type: 'category', path: 'region'}] if name == :main_topic
            end
          end
        end
    ...
    def self.typeahead(q, current_region)
      contexts = {}
      if current_region
        contexts[:region] = [current_region]
      end
