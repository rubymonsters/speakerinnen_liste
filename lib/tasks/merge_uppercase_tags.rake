namespace :tags do
  ...
  task merge: :environment do
    ...
    double_tags = tags.delete_if{|key, value| value.size < 2}

    puts "double_tags size: #{double_tags.size}"
    tags_to_edit = Hash.new
    double_tags.each do |key, tag_pair|
      bad, good = tag_pair.sort{|a,b| a.name <=> b.name}
      tags_to_edit[good] = bad

      tag_pair.sort{|a,b| a.name <=> b.name}.each do |tag|
